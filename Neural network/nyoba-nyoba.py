...
def generate_animation(learning_rate, weight_init, config):
    ...
    weights = [np.load(f'D:/skripsi/data hasil/Variasi 1 Hidden Layer/Learning rate = {learning_rate}, weight init = {weight_init}, no stop condition/3-{config}-3/weight_total_epoch_{i}.npy', allow_pickle=True) for i in range(2)]
    ...
    # Animasi
    ani = FuncAnimation(fig, update, frames=range(1, 400), blit=False, repeat=False)
...
for config in configs:
    generate_animation(learning_rates, weight_inits, config)